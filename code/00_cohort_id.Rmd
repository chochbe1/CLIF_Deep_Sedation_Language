...
sbt_screen_pass <- sbt_screen_imv_times_filt[
  categorical_value %in% c("Pass", "pass")
]
...
meds_time_dependent <- meds_time_dependent %>%
  mutate(med_dose = case_when(
    !grepl("kg", med_dose_unit, ignore.case = TRUE) & !grepl("vasopressin|isoproterenol", med_category, ignore.case = TRUE) ~ med_dose / weight_kg,
