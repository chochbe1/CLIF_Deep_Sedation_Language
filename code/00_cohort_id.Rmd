...
procedural_hours <- adt_imv_filt[
  ...
  .(procedural_hours = as.numeric(difftime(out_dttm, in_dttm, units = "hours"))),
  by = hospitalization_id
]
...
#hosp_id_height_weight <- unique(hosp_height_weight_all$hospitalization_id)

# Create Vitals Table 
height_weight <- c("height_cm", "weight_kg")
vitals_bmi <- vitals_all[hospitalization_id %in% hosp_height_weight_all$hospitalization_id & vital_category %in% height_weight]

# Merge Cohort Info from IMV Hospitalizations with Vitals from All patient_id Hospitalizations
imv_bmi_merge <- merge(hosp_height_weight_all, part_five_imv_runs, 
  by = c("hospitalization_id"), 
  all = TRUE)
setorder(imv_bmi_merge, patient_id, admission_dttm)

# Flag rows with begin_imv and end_imv (i.e the ones corresponding to a cohort admission)
valid_imv_rows <- imv_bmi_merge[!is.na(begin_imv) & !is.na(end_imv)]

# Create a unique key for each hospitalization with valid IMV times
valid_imv_rows[, key_id := .I]

# Join on patient_id and filter only for hospitalizations within +/- one year of IMV hospitalization
result_height_weight <- imv_bmi_merge[valid_imv_rows, on = .(patient_id), nomatch = 0][
  admission_dttm >= (i.admission_dttm - 365*24*60*60) & 
  admission_dttm <= (i.admission_dttm + 365*24*60*60)]

# Filter BMI vitals table to only contain data hospitalizations IDed in previous step
vitals_bmi_filt <- vitals_bmi[hospitalization_id %in% result_height_weight$hospitalization_id]

# Reshape the data to put height/weight into separate columns, using mean to handle duplicates
vitals_bmi_filt <- dcast(vitals_bmi_filt, 
...
vitals_bmi_filt <- vitals_bmi_filt[, .(hospitalization_id, recorded_dttm, weight_kg, height_cm)]
setorder(vitals_bmi_filt, hospitalization_id, recorded_dttm)

# Obtain Patient_IDs associated with BMI Hospitalizations and merge with reshaped table
pat_hosp_list <- imv_bmi_merge[, .(patient_id, hospitalization_id)]
vitals_bmi_patient <- merge(vitals_bmi_filt, pat_hosp_list, by = "hospitalization_id", all = TRUE)
setorder(vitals_bmi_patient, patient_id, recorded_dttm)

# Table of All Height Values - filter out very low values
height_patient <- vitals_bmi_patient[!is.na(height_cm)]
height_patient <- height_patient[, .(patient_id, recorded_dttm, height_cm)]
...
height_hosp <- merge(median_height_per_patient, hosp_height_weight_all, by = "patient_id", all.x = TRUE)
height_hosp <- height_hosp[, .(hospitalization_id, median_height)]

# Create Weight Table and filter out improbably low weights
weight_patient <- vitals_bmi_patient[!is.na(weight_kg)]
weight_patient <- weight_patient[, .(patient_id, hospitalization_id, recorded_dttm, weight_kg)]
weight_patient <- weight_patient[weight_kg >= 35]

# Create table to obtain admission_dttm for all hospitalizations for IMV patients
admission_dttm_table <- hosp_height_weight_all[
  patient_id %in% weight_patient$patient_id, 
  .(hospitalization_id, patient_id, admission_dttm)]

# Add admission_dttm to the weight_patient table
admission_weight_table <- merge(
  admission_dttm_table, weight_patient,
  by = c("hospitalization_id", "patient_id"), 
  all = TRUE)
setorder(admission_weight_table, hospitalization_id, recorded_dttm)

# Calculate absolute time difference between admission_dttm and recorded_dttm
admission_weight_table[, time_diff := abs(difftime(admission_dttm, recorded_dttm, units = "secs"))]

# For each patient_id and admission_dttm, find the row with the minimum time_diff
admission_weight_table[, .SD[which.min(time_diff)], by = .(patient_id, admission_dttm), .SDcols = c("admission_dttm", "weight_kg")]

# Create the final table with patient_id, admission_dttm, and admission_weight
final_weight_table <- admission_weight_table[, .SD[which.min(time_diff)], by = .(patient_id, admission_dttm), .SDcols = c("admission_dttm", "weight_kg")]
final_weight_table[, admission_weight := weight_kg]
final_weight_table <- final_weight_table[, .(patient_id, admission_dttm, admission_weight)]

# Weight Table
weight_hosp <- merge(final_weight_table, hosp_height_weight_all, c("patient_id", "admission_dttm"), all = TRUE)
weight_hosp <- weight_hosp[, .(hospitalization_id, admission_weight)]

# Create BMI Table, heigh_m column, and calculate BMI
patient_bmi_data <- merge(weight_hosp, height_hosp, by = c("hospitalization_id"), 
  all = TRUE)
patient_bmi_data[, height_m := median_height / 100]
patient_bmi_data[, bmi := admission_weight / (height_m^2)]

# Cleaned BMI Table
patient_bmi_data_clean <- patient_bmi_data[, .(hospitalization_id, admission_weight, median_height, bmi)]
