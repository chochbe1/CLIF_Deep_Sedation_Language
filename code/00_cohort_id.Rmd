...
read_data <- function(file_path, select = NULL) {
  ...
  # 2) if we already have a hosp_map and this table has hospitalization_id,
  #    automatically swap in the joined‐encounter ID and block dates
if (exists("hosp_map", envir = .GlobalEnv) &&
    "hospitalization_id" %in% names(dt)) {

  # are both admit/discharge in here?
  has_dates <- all(c("admission_dttm", "discharge_dttm") %in% names(dt))

  if (has_dates) {
    message("🔄 remapping hosp_id, admit & discharge")
    dt <- dt %>%
      left_join(hosp_map, by = c("hospitalization_id" = "old_hosp_id")) %>%
      mutate(
        hospitalization_id = coalesce(new_hosp_id, as.integer(hospitalization_id)),
        admission_dttm     = coalesce(final_admit,     admission_dttm),
        discharge_dttm     = coalesce(final_discharge, discharge_dttm),
        discharge_name     = coalesce(final_discharge_name, discharge_name),
        discharge_category = coalesce(final_discharge_category, discharge_category)
      ) %>%
      dplyr::select(-new_hosp_id, -final_admit, -final_discharge) 
    ...
    dt <- dt %>%
      ...
      dplyr::select(-new_hosp_id, -final_admit, -final_discharge, -final_discharge_name, -final_discharge_category)
  }
}
...
patient_assessments[, `:=`(hospitalization_id = as.integer(hospitalization_id))]
...
lookback <- hospital_diagnosis_cohort %>%
  ...
  mutate(diagnostic_code = str_to_upper(diagnostic_code))
...
#Continuous Meds
specific_med_groups <- c("vasoactives", "sedation", "paralytics", "analgesia")
...
part_eleven_imv_runs[, imv_date := as.Date(begin_imv)]
